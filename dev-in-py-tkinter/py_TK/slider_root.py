...
ray = Tk()
ray.geometry("250x300")
ray.iconbitmap('icony1.ico')
root = LabelFrame(ray,text="frame size",padx=150,pady=180)
root.pack()
#-------------------------------------------
scrol_h = Scale(ray,from_=300,to=600,orient=HORIZONTAL)
scrol_h.pack(anchor=S)
scrol_v = Scale(ray,from_=300,to=600)
...
def scroll():
	...
	ray.geometry(str(scrol_h.get())+"x"+str(scrol_v.get()))
